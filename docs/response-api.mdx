...
Rensa provides a response object (_res_) that extends Node.js's native HTTP response (http.ServerResponse). This means you can still use all default response method, such as res.statusCode, res.setHeader(), and res.end().
