...
export const CodeSnippet = ({ language, copy_able=true, className="", children }) => {
  ...
  const lang = language.toLowerCase() === "js" || language.toLowerCase() === "json" ? "javascript" : language.toLowerCase() === "npm" ? "sh" : language;
  ...
  return (
    <div className={`font-mono ${className}`}>
      <div className="px-3 py-3 md:px-6 md:py-3 bg-primary rounded-t-md max-w-8 md:max-w-32 flex justify-center">
        <span className="uppercase text-xs md:text-md font-bold">{language}</span>
      </div>
